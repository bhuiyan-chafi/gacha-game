...
def rollToWinGacha(request):
    ...
    try:
        player_response = requests.get(
            user_service_url, headers=request.headers, verify=settings.SSL_VERIFY, timeout=5)
    ...
    try:
        response = requests.get(
            gacha_service_url, headers=request.headers, verify=settings.SSL_VERIFY, timeout=5)
    ...
    gacha_response = requests.get(
        gacha_update_url, headers=request.headers, verify=settings.SSL_VERIFY, timeout=5)
    ...
    try:
        with transaction.atomic():
            ...
            balance_response = requests.put(user_service_url, json={
                                            "current_balance": new_balance}, headers=request.headers, verify=settings.SSL_VERIFY, timeout=5)
            ...
            inventory_response = requests.put(
                gacha_update_url, json={"inventory": current_inventory-1}, headers=request.headers, verify=settings.SSL_VERIFY, timeout=5)
...
def createPlayerGachaByPurchase(request):
    ...
    try:
        with transaction.atomic():
            ...
            # Fetch gacha details
            gacha_response = requests.get(
                gacha_url, headers=request.headers, verify=settings.SSL_VERIFY, timeout=5)
            gacha_data = gacha_response.json()
            ...
            gacha_update_response = requests.put(
                gacha_url, json={'inventory': new_inventory}, headers=request.headers, verify=settings.SSL_VERIFY, timeout=5)
    ...
    if request.method == 'GET':
        ...
        try:
            with transaction.atomic():
                ...
                gacha_update_response = requests.put(
                    gacha_url, json={"inventory": "increment"}, headers=request.headers, verify=settings.SSL_VERIFY, timeout=5
                )
